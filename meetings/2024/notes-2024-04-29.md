...
EAO: In addition, because MessageFormat's formatting output is larger, since it returns full messages that may in turn contain the output of the other APIs. So when there is an error in formatting Message Format, the difference between the developer expectation and what the user sees is greater.
