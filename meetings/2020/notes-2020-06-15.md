...
MIH: Good comments received from SMY and NIC. As a short intro about MessageFormat is detailed in the PR doc.
...
SMY: About the second bullet, it's actually a benefit if you see this as a standard. Its not meant to be modified easily.
 
MIH: We could list these benefits, but not sure it belongs in this document. 
 
SMY: From my perspective it would make sense to include it.
...
ECH: I am just agreeing with SMY's suggestion that a list of specific problems in MessageFormat would be useful in this same document.  The document, as it is, does well to list out the high-level problems and explanations.  But specific problems could be useful for future reference.
 
SMY: we should define a core and extend it when possible. Maybe going ahead, but how this is phrased it looks like we can MessageFormat 2 because we cant change MessageFormat 1.
...
SMY: This might be a way to improve the wording. But the localization industry moves very slowly. Something to consider.
...
SMY: I think these points tie into the design principles well.
 
MIH: What does the group think it's actionable items now? Merge 1 and 2? And do you think points from SMY to real examples where MessageFormat is wrong, should it be mentioned in this document?
 
RCA: We don't have a voting mechanism. A +1 in the chat would do.
 
SMY: Jumping to my reply on TCQ. To wrap up my comments, I saw point 2 as mistakes made, but MIH just used them as a high-level reference, right?
 
MIH: Yes.
 
SMY: How about merge point 1 and 2 in one about modular design. And then add point 2 to list mistakes in the old MessageFormat.
...
RCA: Agree with DAF. Next is SMY.
...
SMY: About Design principles, first thanks for comments on goals vs non-goals. I listed 6 goals of the group, 5 design principles and expanded on 9 non goals.
...
-SMY presenting doc (issue #50) with principles-
...
SMY: The discussion about design principle is difficult because it is abstract. Lets take computational vs manual as an example.
...
SMY: In this view the data model only describes the declaration for the specific case.
 
DAF: Rule-based machine translation seems what SMY was referring to with the computational example.  And this is not really feasible to perform for all rules for all languages.
 
SMY: Agree with DAF and that's how Fluent was developed. But I was impressed with how Siri replied to me with correct nouns in my language.
...
SMY: We have spent 3 months discussing abstract topics like goals vs non-goals. We need to move to tangible actions, for example with the principles.
...
SMY: I like that, I think it would work.
 
NIC: Agree, we can get lost with all grammar and language-specific issues. The key is not to corner ourselves, but try to start using something- echo SMY. And yes, we need design principles.
...
SMY: About forward/backward compatibility. Valuable to have different approaches to design principles. It's important to have some compatibility strategy as part of this modular strategy for the standard. DAF can you expand on the roundtrip principle?
...
SMY: I would like to hear this presentation.
